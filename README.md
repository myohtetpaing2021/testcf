...
1. Create a worker named `backend-2d-pro` and paste your 2D Logic code.
2. Create a worker named `backend-3d-pro` and paste your 3D Logic code.
3. Create a worker named `backend-report` and paste your Report Logic code.
4. **Important:** Add `TELEGRAM_BOT_TOKEN` to the Environment Variables of **each** backend worker.
5. **Note:** Do NOT set a webhook for these workers. They are internal services.
...
> **Warning:** Variable names (`BOT_2D`, etc.) must match exactly with the code in `a.js`.
